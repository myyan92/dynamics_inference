...
class physbam_2d(object):
  ...
  def execute_batch(self, state, actions, return_traj=False, **kwargs):
    ...
    return_trajs = [return_traj for a in actions]
    pool = Pool()
    states = pool.starmap(self.execute, zip(state, actions, return_trajs))
    pool.close()
    pool.join()
    return states
